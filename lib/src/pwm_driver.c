...
void pwm_init(void) {
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA,
                         ENABLE);  // GPIO Port A timer clock
  // Configure timer
...
void pwm_stop(void) { TIM_Cmd(PWM_TIMER, DISABLE); }

void pwm_start(void) { TIM_Cmd(PWM_TIMER, ENABLE); }
...
static void (*get_set_CCR_function(uint16_t pwm_channel))(TIM_TypeDef*,
                                                          uint32_t) {
  switch (pwm_channel) {
    case PWM_RIGHT:
      return TIM_SetCompare2;
    case PWM_LEFT:
      return TIM_SetCompare1;
    default:
      return 0;
  }
}
...
void pwm_set_duty_cycle(uint16_t pwm_channel, uint16_t duty_cycle) {
  if (duty_cycle >=
      1010) {  // 101 because 100 does not perfectly generate 100% duty cycles
    duty_cycle = 1010;
  }
  uint32_t t_pulse;
